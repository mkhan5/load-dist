...
//           h_B[i] = (rand() %10)+1;
   ...
   // status = cublasSetVector(n2, sizeof(cublas_C[0]), cublas_C, 1, d_C, 1);
    ...
    //Warming up
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m*(1-scale), n, p, alpha, h_A+(int)(m*(scale)*p), p, h_B, n, beta, cublas_C+(int)(m*(scale)*n), n);
    // Warming up done
    ...
    #pragma omp parallel sections private(nthreads, tid) num_threads(2)
    {
	   ...
	   //  printf("Hello World from thread = %d\n", tid);
        ...
        #pragma omp section
	    {
            ...
            if (status != CUBLAS_STATUS_SUCCESS)
            {
            fprintf(stderr, "!!!! device access error (1 write A)\n");
            //return EXIT_FAILURE;
            }
            ...
            if (status != CUBLAS_STATUS_SUCCESS)
            {
            fprintf(stderr, "!!!! device access error (1 read C)\n");
            //return EXIT_FAILURE;
            }
            if (detailedtime)
            {
    ...
    printf("CUBLAS (%2.3f) + MKL (%2.3f) - Total Time for matrix multiplication:$ %12.3f $s, for threads %d , for matrix size %d x %d \n",scale, 1-scale, (stage1stop-stage1start)+(stoptime_vect-starttime_vect), mkl_get_max_threads(), N,N);
